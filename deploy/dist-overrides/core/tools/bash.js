...
export function createBashToolDefinition(cwd, options) {
    ...
    return {
        name: "bash",
        label: "bash",
        messageDescription: (getToolPrompt('bash', 'messageDescription') || ''),
        promptSnippet: getToolPrompt('bash', 'snippet'),
        parameters: bashSchema,
        async execute(_toolCallId, { command }, signal, onUpdate, _ctx) {
            // ── 破坏锁：git checkout/reset/clean 必须用户手动执行 ──
            const DESTRUCTIVE_GIT = /\bgit\s+(checkout(?!\s+-b)|reset|clean|revert|push\s+(-f|--force)|rm|branch\s+-D)/;
            if (DESTRUCTIVE_GIT.test(command)) {
                return {
                    content: [{ type: "text", text: `⚠️ 破坏性 git 命令被拦截。请手动执行：\n\n  ${command}\n\n模型不能直接跑 checkout/reset/clean/revert/force-push/rm/branch -D。` }],
                    details: {},
                    isError: true,
                };
            }
            ...
            return new Promise((resolve) => {
                ...
                child.on("close", (code) => resolve({ content: [{ type: "text", text: out.slice(0, 50000) || "(no output)" }], details: { exitCode: code } }));
            });
