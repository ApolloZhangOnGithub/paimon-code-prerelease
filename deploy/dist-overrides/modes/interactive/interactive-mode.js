...
export class InteractiveMode {
    ...
    prefixAutocompletemessageDescription(messageDescription, sourceInfo) {
        const sourceTag = this.getAutocompleteSourceTag(sourceInfo);
        if (!sourceTag) {
            return messageDescription;
        }
        return messageDescription ? `[${sourceTag}] ${messageDescription}` : `[${sourceTag}]`;
    }
    ...
    createBaseAutocompleteProvider() {
        ...
        const slashCommands = BUILTIN_SLASH_COMMANDS.map((command) => ({
            name: command.name,
            messageDescription: command.messageDescription,
        }));
        ...
        if (modelCommand) {
            modelCommand.getArgumentCompletions = (prefix) => {
                ...
                return filtered.map((item) => ({
                    value: item.label,
                    label: item.id,
                    messageDescription: item.provider,
                }));
            };
        }
        ...
        const templateCommands = this.session.promptTemplates.map((cmd) => ({
            name: cmd.name,
            messageDescription: this.prefixAutocompletemessageDescription(cmd.messageDescription, cmd.sourceInfo),
        ...
        if (this.settingsManager.getEnableSkillCommands()) {
            for (const skill of this.session.resourceLoader.getSkills().skills) {
                ...
                skillCommandList.push({
                    name: commandName,
                    messageDescription: this.prefixAutocompletemessageDescription(skill.messageDescription, skill.sourceInfo),
                });
            }
        }
    ...
    async init() {
        ...
        // pi-coding-master: 始终显示 logo，不显示帮助横幅
        {
            const logo = theme.bold(theme.fg("accent", APP_NAME)) + theme.fg("dim", ` v${this.version}`);
            // Build startup instructions using keybinding hint helpers
            const hint = (keybinding, messageDescription) => keyHint(keybinding, messageDescription);
    ...
    showLoadedResources(options) {
        ...
        if (showListing) {
            ...
            // pi-coding-master: 跳过 Skills 显示（静默启动）
            // const skills = skillsResult.skills;
            // if (skills.length > 0) { ... addLoadedSection("Skills", ...); }
            ...
            // pi-coding-master: 跳过 Extensions 显示（静默启动）
            // if (extensions.length > 0) { ... addLoadedSection("Extensions", ...); }
        ...
        if (showDiagnostics) {
            ...
            const shortcutDiagnostics = this.session.extensionRunner.getShortcutDiagnostics()
                .filter(d => d.message && !d.message.includes("shortcut 'escape'")); // pi-coding-master: 忽略 escape 快捷键冲突
    ...
    setupKeyHandlers() {
        ...
        this.defaultEditor.onEscape = () => {
            // pi-coding-master: 标记 ESC 刚被按下，agent_end 据此区分「用户按 ESC」vs「语音 steer 导致的 abort」
            // 总是先设 flag，然后根据当前状态做对应的停行动作。
            // 不在 streaming 时也要 abort——比如 wait 期间按 ESC 应取消 wait，而不是光设个 flag 等 timer 到点。
            global.__piEscJustPressed = true;
            this.restoreQueuedMessagesToEditor({ abort: true });
    ...
    async handleEvent(event) {
        ...
        switch (event.type) {
            ...
            case "agent_end":
                ...
                // Don't stop loader on agent_end — pi-coding-master auto-continues, stopping causes flicker
                if (this.streamingComponent) {
    ...
    addMessageToChat(message, options) {
        switch (message.role) {
            ...
            case "custom": {
                if (message.display) {
                    const renderer = this.session.extensionRunner.getMessageRenderer(message.messageType);
                    ...
                    if (content.type === "toolCall") {
                        ...
                        if (message.stopReason === "aborted" || message.stopReason === "error") {
                            ...
                            if (message.stopReason === "aborted") {
                                // pi-coding-master: suppress "Operation aborted" — steers/auto-aborts are normal in continuous mode
                                // Only show specific errors
                                if (message.errorMessage && message.errorMessage !== "Operation aborted" && message.errorMessage !== "Request was aborted") {
    ...
    async shutdown(options) {
        ...
        this.stop();
        await this.runtimeHost.dispose();
        // pi-coding-master: 不打印 "To resume this session" —— pi-coding-master 每次都是新 session（续命靠 context.md/记忆，不靠 resume），
        // 这行 resume 命令对它无意义、还干扰。整体禁掉。
        const resumeCommand = formatResumeCommand(this.sessionManager);
        if (false && resumeCommand) {
...
`;
            for (const [key, shortcut] of shortcuts) {
                const messageDescription = shortcut.messageDescription ?? shortcut.extensionPath;
                const keyDisplay = formatKeyText(key, { capitalize: true });
                hotkeys += `| \`${keyDisplay}\` | ${messageDescription} |\n`;
            }
        }
        this.chatContainer.addChild(new Spacer(1));
        this.chatContainer.addChild(new DynamicBorder());
