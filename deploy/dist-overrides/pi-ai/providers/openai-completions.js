...
export const streamOpenAICompletions = (model, context, options) => {
    ...
    (async () => {
        ...
        try {
            ...
            // pi-coding-master: 打断(steer/abort)后立刻通知云端停止生成。
            // signal 一 abort 就 cancel 这条 SDK stream(断连) —— 不等下一个 chunk。
            // 否则下面 for-await 会把 deepseek 整段生成读完(服务器一路生成、一路计费),
            // 而 aborted 的 run 我们记 usage=0 → 这笔整段生成的钱看不见(隐形烧钱)。
            if (options?.signal) {
                const onPiAbort = () => { try { openaiStream.controller?.abort(); } catch { } };
                globalThis.__piAbort = () => { globalThis.__piStreamStop = true; try { openaiStream.controller?.abort(); } catch { } };
                if (options.signal.aborted)
                    onPiAbort();
                else
                    options.signal.addEventListener("abort", onPiAbort, { once: true });
            }
            stream.push({ type: "start", partial: output });
            let textBlock = null;
            let thinkingBlock = null;
            let hasFinishReason = false;
            globalThis.__piStreamStop = false;
            ...
            const ensureTextBlock = () => {
                if (!textBlock) {
                    textBlock = { type: "text", text: "", ts_start: Date.now() };
            ...
            for await (const chunk of openaiStream) {
                // pi-coding-master: next handler 调 __piAbort 后设标志，下次迭代即断流
                if (globalThis.__piStreamStop) break;
                // pi-coding-master: 打断 → 立刻停读并退出。break 触发异步迭代器 return() → 取消底层流 → 断连 → 云端停。
                if (options?.signal?.aborted) {
                    try { openaiStream.controller?.abort(); } catch { }
                    break;
                }
...
export function convertMessages(model, context, compat) {
    ...
    for (let i = 0; i < transformedMessages.length; i++) {
        ...
        if (compat.requiresAssistantAfterToolResult && lastRole === "toolResult" && msg.role === "user") {
            params.push({
                role: "assistant",
                content: P('processedToolResults'),
            });
        }
        if (msg.role === "user") {
