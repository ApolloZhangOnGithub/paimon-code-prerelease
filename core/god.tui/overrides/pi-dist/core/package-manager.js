...
function addIgnoreRules(ig, dir, rootDir) {
    ...
    for (const filename of IGNORE_FILE_NAMES) {
        ...
        catch(e) { try { require("fs").appendFileSync("/tmp/paimon-catch-errors.log", "[1E001] " + (e?.stack||e) + "\n"); } catch {} }
    }
}
...
function collectAutoExtensionEntries(dir) {
    ...
    try {
        ...
        for (const entry of dirEntries) {
            ...
            if (entry.name === "node_modules")
                continue;
            // paimon-code: 跳过【禁用/备份】的扩展目录 —— 停用一个扩展只需就地改名,不必移出 extensions/
            // 约定:_开头 / *.bak / *.stale / *.old / 名字含 disabled = 已停用,发现阶段直接忽略
            // (扩展真正的发现入口是这里 collectAutoExtensionEntries,不是 core/extensions/loader.js)
            if (entry.name.startsWith("_") ||
                /\.(bak|stale|old|removed|orig)\b/i.test(entry.name) ||
                /disabled/i.test(entry.name))
                continue;
            const fullPath = join(dir, entry.name);
            let isDir = entry.isDirectory();
            let isFile = entry.isFile();
