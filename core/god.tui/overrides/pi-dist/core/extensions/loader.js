...
async function loadExtensionModule(extensionPath) {
    // paimon-code: 先把扩展入口解析成 realpath 再交给 jiti。
    // 否则经软链加载的扩展(如 device → Codebase/core/technology.local.mobile)，其 #subpath-imports
    // (#hands_terminal/#world_space 等)会从软链路径 ~/.paimon/agent/extensions/device/ 向上找 package.json
    // → 找不到 Codebase/core/package.json → 报 "Cannot find module '#hands_terminal'"。
    // realpath 后 jiti 从真实位置向上走，正确命中 core/package.json 的别名表（paimon-code 这种
    ...
    catch(e) { try { require("fs").appendFileSync("/tmp/paimon-catch-errors.log", "[64001] " + (e?.stack||e) + "\n"); } catch {} }
    // paimon-code: jiti disabled — use native Node import
    // 缓存失效：加 ?t=timestamp 强制 Node 重新执行模块顶层代码
    // 模块内部 import (#ribosome 等) 不受影响——那些包本身不变
...
function discoverExtensionsInDir(dir) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            // paimon-code: 跳过【禁用/备份】的扩展目录 —— 名字以 . 或 _ 开头,或带 .disabled/.bak/.stale/.old。
...
export function startExtensionWatcher(extDirs) {
  ...
  for (const dir of extDirs) {
    ...
    } catch(e) { try { require("fs").appendFileSync("/tmp/paimon-catch-errors.log", "[64002] " + (e?.stack||e) + "\n"); } catch {} }
  }
}
