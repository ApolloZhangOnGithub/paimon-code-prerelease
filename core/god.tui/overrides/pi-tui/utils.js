...
export function applyBackgroundToLine(line, width, bgFn) {
    ...
    if (paddingNeeded <= 0) return bgFn(line);
    const padding = " ".repeat(paddingNeeded);
    // 把行尾的 OSC 8 close / SGR reset 等不可见序列切出来，空格塞在它们之前
    // 避免尾部空格落在 OSC 8 close 之后导致部分终端超链接下划线异常
    const tailSeq = extractTailAnsiOsc(line);
    const body = tailSeq ? line.slice(0, line.length - tailSeq.length) : line;
    return bgFn(body + padding + (tailSeq || ""));
}

// 提取行尾的 ANSI SGR reset + OSC 8 close 序列（不可见序列链）
const TAIL_ANSI_OSC_RE = /((?:\x1b\][0-9;]*\x1b\\|\x1b\[[0-9;]*m)+)$/;
export function extractTailAnsiOsc(line) {
    const m = line.match(TAIL_ANSI_OSC_RE);
    return m ? m[0] : "";
}
/**
