...
export class ProcessTerminal {
    ...
    setupStdinBuffer() {
        ...
        this.stdinBuffer.on("data", (sequence) => {
            if (process.env.INPUTPS03 === "0") return;
        ...
        this.stdinDataHandler = (data) => {
            if (process.env.INPUTPS02 === "0") return;
    ...
    queryAndEnableKittyProtocol() {
        ...
        if (process.env.INPUTPS01 !== "0") process.stdout.write(KITTY_KEYBOARD_PROTOCOL_QUERY);
    }
    ...
    forwardInputSequence(sequence) {
        if (!this.inputHandler)
            return;
        if (process.env.INPUTPS04 === "0") return;
