    ...
    renderToken(token, width, nextTokenType, styleContext) {
        ...
        switch (token.type) {
            case "heading": {
                ...
                // paimon-code: 标题一律【粗体】，去掉 theme.heading 的黄色；h1 额外加下划线作层级区分。
                let headingStyleFn;
                if (headingLevel === 1) {
                ...
                // paimon-code: 不再为 level>=3 打印字面 "### " 前缀（之前导致 ### 看着像没渲染）。
            ...
            case "code": {
                ...
                const fmtLineNo = (n) => String(n).padStart(gutterWidth) + " │ ";
                if (this.theme.highlightCode) {
                    const highlightedLines = this.theme.highlightCode(token.text, token.lang);
                    for (let i = 0; i < highlightedLines.length; i++) {
                        lines.push(`${indent}${fmtLineNo(i + 1)}${highlightedLines[i]}`);
                    }
                }
                else {
                    const codeLines = token.text.split("\n");
                    for (let i = 0; i < codeLines.length; i++) {
                        lines.push(`${indent}${fmtLineNo(i + 1)}${this.theme.codeBlock(codeLines[i])}`);
                    }
                }
            ...
            case "hr":
                lines.push(this.theme.hr("─".repeat(width)));
                if (nextTokenType && nextTokenType !== "space") {
