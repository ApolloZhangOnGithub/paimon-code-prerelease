...
export class Editor {
    ...
    render(width) {
        ...
        // EDITOR01=上border EDITOR02=内容行 EDITOR03=下border
        // 设 EDITORXX=0 关闭对应部分，默认全开
        const _e01 = process.env.EDITOR01 !== "0";
        const _e02 = process.env.EDITOR02 !== "0";
        const _e03 = process.env.EDITOR03 !== "0";
        const _push = (line) => result.push(line);
        // Render top border (with scroll indicator if scrolled down)
        if (this.scrollOffset > 0) {
            const indicator = `─── ↑ ${this.scrollOffset} more `;
            const remaining = width - visibleWidth(indicator);
            if (remaining >= 0) {
                if (_e01) result.push(this.borderColor(indicator + "─".repeat(remaining)));
            }
            else {
                if (_e01) result.push(this.borderColor(truncateToWidth(indicator, width)));
            }
        }
        else {
            if (_e01) result.push(horizontal.repeat(width));
        }
        ...
        if (linesBelow > 0) {
            ...
            const remaining = width - visibleWidth(indicator);
            if (_e03) result.push(this.borderColor(indicator + "─".repeat(Math.max(0, remaining))));
        }
        else {
            if (_e03) result.push(horizontal.repeat(width));
        }
