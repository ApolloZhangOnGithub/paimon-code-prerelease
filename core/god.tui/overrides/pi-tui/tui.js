...
export class TUI extends Container {
    ...
    handleInput(data) {
        if (process.env.INPUTPS05 === "0") return;
        ...
        if (this.focusedComponent?.handleInput) {
            ...
            if (process.env.INPUTPS06 !== "0") this.focusedComponent.handleInput(data);
            if (process.env.INPUTPS07 !== "0") this.requestRender();
        }
    }
    ...
    doRender() {
        ...
        // paimon-code: 超宽行折行（挂起缩进）— 暂时禁用排查输入问题
        // if (this._wrapCache?.width === width && this._wrapCache?.ref === newLines) {
        //     newLines = this._wrapCache.result;
        // } else {
        //     const wrapped = wrapHanging(newLines, width, { visibleWidth, sliceWithWidth, sliceByColumn, isImageLine });
        //     this._wrapCache = { width, result: wrapped, ref: newLines };
        //     newLines = wrapped;
        // }
