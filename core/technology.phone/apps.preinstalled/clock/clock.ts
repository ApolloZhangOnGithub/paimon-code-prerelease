// apps.preinstalled/clock/clock.ts — Clock tool (纯 pi 实现)
// 闹钟 / 秒表 / 计时器
...
export async function clockCmd(args: any, _ctx: any, personDir: string): Promise<{ content: any[]; details: any }> {
  ...
  if (a==="stopwatch_start") { state={...state,sw_status:"running",sw_accumulated:0,sw_started_at:Date.now(),sw_laps:[]}; return {content:[{type:"text",text:"秒表已启动"}],details:{}}; }
  if (a==="stopwatch_pause") { if(state.sw_status!=="running") return {content:[{type:"text",text:"秒表未在运行"}],details:{}}; state.sw_accumulated=swElapsed(); state.sw_status="stopped"; state.sw_started_at=0; return {content:[{type:"text",text:`暂停 (${fmt(state.sw_accumulated)})`}],details:{}}; }
  if (a==="stopwatch_resume") { if(state.sw_status!=="stopped") return {content:[{type:"text",text:"秒表未暂停"}],details:{}}; state.sw_status="running"; state.sw_started_at=Date.now(); return {content:[{type:"text",text:"已继续"}],details:{}}; }
  if (a==="stopwatch_reset") { state={...state,sw_status:"reset",sw_accumulated:0,sw_started_at:0,sw_laps:[]}; return {content:[{type:"text",text:"已重置"}],details:{}}; }
  if (a==="stopwatch_lap") { if(state.sw_status!=="running") return {content:[{type:"text",text:"秒表未在运行"}],details:{}}; const e=swElapsed(),lt=state.sw_laps.length>0?state.sw_laps[state.sw_laps.length-1].total:0; state.sw_laps.push({number:state.sw_laps.length+1,split:Math.round((e-lt)*10)/10,total:Math.round(e*10)/10}); return {content:[{type:"text",text:`计次#${state.sw_laps.length}`}],details:{laps:state.sw_laps}}; }
  ...
  if (a==="timer_start") { const m=parseInt(p.minutes)||5; state.timer_status="running"; state.timer_label=p.label||`${m}分钟`; state.timer_total_secs=m*60; state.timer_started_at=Date.now(); return {content:[{type:"text",text:`已启动: ${state.timer_label}`}],details:{}}; }
  if (a==="timer_cancel") { state.timer_status="idle"; state.timer_started_at=0; return {content:[{type:"text",text:"已取消"}],details:{}}; }
