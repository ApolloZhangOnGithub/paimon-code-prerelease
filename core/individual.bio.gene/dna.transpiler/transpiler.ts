// dna.transpiler/transpiler.ts
// ── RNA polymerase ──────────────────────────────────────────────────────────
// 把基因组（individual.bio.gene/dna.*/promotor.dna + coded.dna）转录成 RNA（individual.bio.gene/rna/rna.json）。
...
//
// 运行：  bun individual.bio.gene/dna.transpiler/transpiler.ts        （从 Codebase/core 根目录）
// 产物：  individual.bio.gene/rna/rna.json

import { readFileSync, writeFileSync, readdirSync } from "node:fs";
import { resolve, dirname } from "node:path";

const GENE_ROOT = resolve(dirname(new URL(import.meta.url).pathname), "..");
...
function compile(
  ...
  // ── 反向校验: coded.dna 里每个块都必须被至少一个 func 引用 ──
  ...
  // ── 检测 organs/ 下有 .ts 文件但未在 promotor.dna 声明的目录 ──
  try {
  ...
  return {
    generatedFrom: ["individual.bio.gene/dna.promotor/promotor.dna", "individual.bio.gene/dna.coded/coded.dna"],
    note: "GENERATED by dna.transpiler/transpiler.ts — 不要手改，改 .dna 后重新转录。",
    sessions: p.sessions,
    modes: rnaModes,
    tags: rnaTags,
    funcs: rnaFuncs,
...
// ── main ─────────────────────────────────────────────────────────────────────
function main() {
  const promotorText = readFileSync(`${GENE_ROOT}/dna.promotor/promotor.dna`, "utf8");
  const codedText = readFileSync(`${GENE_ROOT}/dna.coded/coded.dna`, "utf8");

  const parsed = parsePromotor(promotorText);
  const coded = parseCoded(codedText);
  const rna = compile(parsed, coded);

  writeFileSync(`${GENE_ROOT}/rna/rna.json`, JSON.stringify(rna, null, 2) + "\n");

  const F = Object.keys(rna.funcs).length;
  const C = Object.keys(coded).length;
  console.log(`transcribed → individual.bio.gene/rna/rna.json  (${F} funcs, ${C} coded prompts)`);
